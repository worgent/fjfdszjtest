		...
		function showSearch(){
			...
			Ext.onReady(function() {
			    var schForm = new Ext.form.FormPanel({
			        ...
			        submit:function(){
			        	this.getEl().dom.action='/carmanage/expedite/expediteex.shtml';
			       ...
			       items: [belongCompanyTree,{
			            ...
			            fieldLabel: '用车人',
			            name: 'search.useCarManName',
			            width:100  // 设置宽度，百分比的需加‘号
			        },{
			            fieldLabel: '驾驶员',
			            name: 'search.driverNameex',
			            width:100  // 设置宽度，百分比的需加‘号
			        },{
			        	xtype: 'datefield',
			        	format: 'Y-m-d',
		...
		<c:if test="${param.action != 'report'}">
			Ext.onReady(function(){
				...
				new Ext.Button({
			        text: '添 加',
			        handler: function(){
			        	parent.addTab('添加临时派车','editExpediteex','/carmanage/expedite/expediteex.shtml?actionType=new','NO');
			        }
		...
		function fun_delete(expediteCarId){
		    if (!confirm('您确定删除该信息!')){
				return;
			   	...
			   	var url = '/carmanage/expedite/expediteex.shtml?actionType=delete&search.expediteCarId='+expediteCarId;
				try{
					...
					if (null != root){
						...
						if (0 < rowSet.item(0).selectSingleNode("value").text){
							alert("删除临时派车信息，操作成功！");
							parent.document.ifrm_ExpediteCarBookingEx.window.location.reload();
						}else{
							alert("删除临时派车信息，操作失败！");
						}
					}
		...
		<tt:row >
			<tt:col name="单据号" width="120">
				<ww:if test="null != expedite_car_id">
					<a href="javascript:parent.addTab('临时派车信息', 'viewExpediteEx', '/carmanage/expedite/expediteex.shtml?actionType=view&search.expediteCarId=<ww:property value="expedite_car_id"/>&search.expediteapplyId=<ww:property value="expediteapply_id"/>','NO')">
					...
					<ww:if test="null != expedite_car_id">
						<ww:if test="expedite_state == 1">
							<tt:authority value="XGPCXX">
								<a href="javascript:parent.addTab('修改临时派车信息', 'editExpediteEx', '/carmanage/expedite/expediteex.shtml?actionType=edit&search.expediteCarId=<ww:property value="expedite_car_id"/>&search.expediteapplyId=<ww:property value="expediteapply_id"/>','NO')">修改</a>
							</tt:authority>
						</ww:if>
					</ww:if>
				</tt:col>
