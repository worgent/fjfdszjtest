				...
				try{
					...
					if (null != root){
						...
						if(0<rowSet.item(0).selectSingleNode("value").text){
						   ...
						   window.location.href='<%=path%>/system/message.do?action=recmessage';
						    //});
							...
							<s:iterator id="revList" value="%{pageList.objectList}">
								<tr id="thread_<s:property value="#revList.ID" />" class="">
									...
									<td class="profile_pic">
										<a href="<%=path%>/system/message.do?action=viewmessage&search.pid=<s:property value="#revList.ID" />"><img
												src="***.jpg"
												width="50" />
										</a>
									</td>
									<td class="name_and_date">
										...
										<s:property value="#revList.SENDNAME" />
										</a> </span>
