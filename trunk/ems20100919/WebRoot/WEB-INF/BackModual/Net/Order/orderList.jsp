  ...
  <div id="precaddressTip"  ></div>						
   ...
   <td style="text-align:center;" class="tdex">
      ...
      <s:if test="%{order.ORDERINGVALUE.substring(7,8)==1}">
			<input type="button" id="btnupdate" name="btnupdate"  onclick="javascript:save('<s:property value="%{action}"/>');"  value="保存不下单"  alt="保存订单" class="outStyle" onmouseover="this.className='overStyle'"  onmouseout="this.className='outStyle'"  />
	  </s:if>
	  <s:if test="%{order.ORDERINGVALUE.substring(8,9)==1}">
 	        &nbsp; &nbsp; &nbsp;<input type="button" id="btnupdateauding" name="btnupdateauding"  onclick="javascript:save('clientsaveadd');"  value="下单" alt="保存订单并通知EMS上门取件" class="outStyle" onmouseover="this.className='overStyle'"  onmouseout="this.className='outStyle'"  />
 	  </s:if>
...
<script language="javascript" type="text/javascript">	
		...
		$("#ptel").formValidator({onshow:" ",onfocus:" ",oncorrect:" "}).regexValidator({regexp:"^\\s+$|^$|^[0-9\\-]+$",onerror:def_Errortel});
        $("#pmobile").formValidator({onshow:" ",onfocus:" ",oncorrect:" "}).regexValidator({regexp:"^\\s+$|^$|^(13|15|18)[0-9]{9}$|^\\d{10,12}$",onerror:def_Errormobile});
