...
//			celltitle = rowtitle.getCell((short) 0);//获得第一行第一个单元格的引用
			...
			for (int i = 0; i < colName.length; i++)//根据获取到的列头数据，循环设置列头的内容
			{
				...
				var=new HSSFRichTextString(colName[i]);
				cellfield.setCellValue(var);
				//setFieldStyle(wb, cellfield);//设置列头格式
				int xx = cellfield.getRichStringCellValue().length();
			...
			while (rs.next()) {
				...
				for (int i = 0; i < fields.length; i++) {
					HSSFCell cellbody = rowbody.createCell( i);
					//cellbody.setEncoding(HSSFCell.ENCODING_UTF_16);
					var=new HSSFRichTextString(PubFunction.getNulltoStr(rs.getString(fields[i])));
					cellbody.setCellValue(var);
				}
				count++;
			}
			//=======================页尾========================
