...
//  ????????
// -------------------------------------------------------------------
function checkTheNode(from,to){
	//??????from??????
	var optionsFrom = new Array();
	if (hasOptions(from)) {
		for (var i=0; i<from.options.length; i++) {
		var o = from.options[i];
			if (o.selected) {
				if(o.value!="" && o.value!="headerKey"){
					optionsFrom[i] = from.options[i];
				}
			}
		}
	}
	//??????to??????
	var optionsTo = new Array();
	if (hasOptions(to)) {
		for (var i=0; i<to.options.length; i++) {
		var o = to.options[i];
			optionsTo[i] = to.options[i];
		}
	}
	...
	for (var i=0; i<from.options.length; i++) {
		var o = from.options[i];
		if(o.selected){
			for(var j=0;j<optionsFrom.length;j++){
				if(optionsFrom[j]!=null){
					//alert("1:"+o.value+"\n2:"+optionsFrom[j].value+"\n");
					//alert("L1:"+o.value.length+"\nL2:"+optionsFrom[j].value.length+"\n");
					if(o.value.length!=optionsFrom[j].value.length){
						var str1 = o.value;
						var str2 = optionsFrom[j].value.substring(0,o.value.length);
						var str3 = optionsFrom[j].value;
						if(str1==str2){
							optionsFrom[j]=null;
							//alert("Delete:\n"+o.value+"\n"+str3+"\nLength"+optionsFrom.length);
						}
					}
				}
			}
	...
	for (var i=0; i<from.options.length; i++) {
		var o = from.options[i];
		if(o.selected){
			for(var j=0;j<optionsTo.length;j++){
				if(optionsTo[j]!=null){
					//alert("1:"+o.value+"\n2:"+optionsTo[j].value+"\n");
					//alert("L1:"+o.value.length+"\nL2:"+optionsTo[j].value.length+"\n");
					if(o.value.length>optionsTo[j].value.length){
						var str1 = optionsTo[j].value;
						var str2 = o.value.substring(0,str1.length);
						var str3 = o.value;
						if(str1==str2){
							optionsTo[j]=null;
							//alert("Delete:\n"+o.value+"\n"+str3+"\nLength"+optionsFrom.length);
						}
					}else if(o.value.length<optionsTo[j].value.length){
						var str1 = o.value;
						var str2 = optionsTo[j].value.substring(0,str1.length);
						var str3 = optionsTo[j].value;
						if(str1==str2){
							optionsTo[j]=null;
							//alert("Delete:\n"+o.value+"\n"+str3+"\nLength"+optionsFrom.length);
						}
	...
	for(var i=0;i<optionsTo.length;i++){
		...
		if(io!=null){
			for(var j=0;j<optionsFrom.length;j++){
				var jo = optionsFrom[j];
				if(jo!=null){
					if(io.value==jo.value){
						optionsFrom[j]=null;
					}
				}//end if
			}//end for
	...
	//Remove All Note for to
	var i_to = to.options.length;
	...
	for(var i=0;i<optionsTo.length;i++){
		...
		if(io!=null){
			if (!hasOptions(to)) { var index = 0; } else { var index=to.options.length; }
			var t_text=io.text.split("|- ");
			if(t_text.length>1){
				t_text = t_text[1];
			}
			to.options[index] = new Option( t_text, io.value, false, false);
		}//end if
	}//end for	
	for(var i=0;i<optionsFrom.length;i++){
		var io = optionsFrom[i];
		if(io!=null){
			if (!hasOptions(to)) { var index = 0; } else { var index=to.options.length; }
			var t_text=io.text.split("|- ");
			if(t_text.length>1){
...
function removeSelectedOptions(from) { 
	...
	if (from.type=="select-one") {
		...
		for (var i=(from.options.length-1); i>=0; i--) { 
			var o=from.options[i]; 
			if (o.selected && o.value!="doubleHeaderKey") { 
				from.options[i] = null; 
				} 
			}
		}
