...
function writeIframe()
{
    ...
    "    </td><td align=center>"+
    "	<input name=today type=button value='今天' onfocus='this.blur()' style='width:30' title='当前日期&#13;快捷键：T'"+
    "    onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\"><input name=clear type=button value='清除' onfocus='this.blur()' title='清除时间&#13;快捷键：C' style='width:30' onclick=\"parent.WebCalendar.objExport.value='';parent.hiddenCalendar();\">"	+
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(pobject=="null" || pobject==undefined){
        	...
        	{
        		if(ptype=="0"){
        		fun_attr_date(pobject,pobject.value);
	        	}
	           if(ptype=="1"){
	        		fun_attr_enddate(pobject,pobject.value);
	           }
