...
public class FrontLoginAction extends BaseAction {
	...
	public String execute() {
		...
		if ("".equals(super.getAction()) || null == super.getAction()|| "index".equals(super.getAction())) {
			...
			try {
				...
				pcode = pcode.toUpperCase();//全转大写忽略大小写方案
			} catch (Exception e) {
			...
			try {
				...
				if (sessioncode.equalsIgnoreCase(search.get("pverifycode")
						.toString())) {
					// 验证码的处理
					String pid=frontloginFacade.getpid();
			...
			if (i == 0) {
				...
				this.addActionMessage(this.getText("注册成功"));
			}
			search.put("phide", i);
	...
	public String menu() {
		...
		strhtml=frontloginFacade.menu(hs);
		search.put("menu", strhtml);
		return "menu";
	}
