...
function ejectnum(){   
  if ( ((event.keyCode > 47) && (event.keyCode < 58)) ||      
	...
	<body>
		<form action="/custom.do?actionType=update"	onsubmit="return checkSubmit()" method="post">
	<table  border="0" cellpadding="0" align="center" style="border-collapse: collapse" width="700" height="100%" id="table15">
	  <tr>
		...
		<strong>修改自定义模板</strong>						<s:hidden name="custom.id" value="%{custom.id}"></s:hidden>
						<s:hidden name="custom.tableName" value="%{custom.tableName}"></s:hidden>
		</td>
	  </tr>
				<tr>
					<td align="left" class="tdex" style="font-size: 12px">
						模板类别：<select name="custom.patternType" id="patternType" style="editcss">
							<option value="1"
								<s:if test="%{custom.patternType==1}">selected="selected"</s:if>>
								日常工作
							</option>
							<option value="2"
								<s:if test="%{custom.patternType==2}">selected="selected"</s:if>>
								督办工作
							</option>
							<option value="3"
								<s:if test="%{custom.patternType==3}">selected="selected"</s:if>>
								事项申报
							</option>
						</select>
					</td>
					<td align="left" class="tdex" style="font-size: 12px">
						模&nbsp;板&nbsp;名：
						<input type="text" id="patternName" name="custom.patternName"
							value="<s:property value='%{custom.patternName}'/>"
							style="editcss" />
					</td>
					<td align="left" class="tdex" style="font-size: 12px">
						状&nbsp;&nbsp;&nbsp;&nbsp;态：
						<select name="custom.state" style="editcss">
							<option value="1"
								<s:if test="%{custom.state==1}">selected="selected"</s:if>>
								启用
							</option>
							<option value="0"
								<s:if test="%{custom.state==0}">selected="selected"</s:if>>
								停用
							</option>
						</select>
					</td>
				...
				<tr>
					<td align="left" class="tdex" style="font-size: 12px">
						事&nbsp;&nbsp;&nbsp;&nbsp;件：<s:checkboxlist list="eventValues" listKey="key" listValue="value"
							name="event" theme="womobile" />
					</td>
					<td align="left" class="tdex" style="font-size: 12px">
						创建时间：
						<s:property value="%{custom.MakeDatetime}" />
					</td>
					<td align="left" class="tdex" style="font-size: 12px">
						创&nbsp;建&nbsp;人：
						<s:property value="%{custom.maker}" />
					</td>
				</tr>
				<tr>
					<td colspan=3 align="left" class="tdex" style="font-size: 12px">
						描&nbsp;&nbsp;&nbsp;&nbsp;述：
						<textarea id="remark" name="custom.remark" cols="30" rows="5"
							class="editcss"
							style="width: 500px; height: 40px; text-align: left">
					<s:property value="%{custom.remark}" />
					</textarea>
					</td>
         ...
         <tbody id="maintaintbody">
          <tr>
			 ...
			 <td width="12%" height="18" background="<%=path%>/css/images/tab_14.gif" class="STYLE1"><div align="center" class="STYLE2 STYLE1"><input id="addButton" name="addButton" type="button" value="添加" onclick="addrow()" class="btn" /></div></td>
											...
											<s:if test="customItemList!=null">
												<input type="hidden" name="num"
													value="<s:property value="customItemList.size()"/>">
											</s:if>
											<s:if test="customItemList==null">
												<s:hidden id="num" name="num" value="0" />
											</s:if>
											<s:iterator id="ci" value="%{customItemList}" status="index">
												<%
													double a = Math.random();
												%>
												<tr>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<input name='custom.fielddesc'
															value="<s:property value="#ci.fielddesc"/>"
															 onkeydown='return ejectnum();'
															class='' size='20' /></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<select class='editcss' id='type<%=a%>'
															onchange='change("<%=a%>")' name='custom.fieldtype'>
															<option value='0'>
																---请选择---
															</option>
															<option value='varchar'
																<s:if test="#ci.fieldtype=='varchar'">selected</s:if>>
																字符
															</option>
															<option value='number'
																<s:if test="#ci.fieldtype=='number'">selected</s:if>>
																数字
															</option>
															<option value='date'
																<s:if test="#ci.fieldtype=='date'">selected</s:if>>
																日期
															</option>
															<option value='enum'
																<s:if test="#ci.fieldtype=='enum'">selected</s:if>>
																枚举
															</option>
															<option value='position'
																<s:if test="#ci.fieldtype=='position'">selected</s:if>>
																位置
															</option>
															<option value='photo'
																<s:if test="#ci.fieldtype=='photo'">selected</s:if>>
																照片
															</option>
														</select></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<input name='custom.fieldlength' id='len<%=a%>'
															onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
															value="<s:property value="#ci.fieldlength"/>"
															 class='editcss' size='5' value='' /></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<select name='custom.fieldenum' id='enum<%=a%>'
															class='editcss'>
															<option value=''></option>
															<s:iterator id='el' value='%{enumList}'>
																<option value='<s:property value='#el.id'/>'
																	<s:if test="#ci.fieldenum==#el.id">selected</s:if>>
																	<s:property value='#el.name' />
																</option>
															</s:iterator>
														</select></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<input name='custom.fieldseqn'
															onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
															value="<s:property value="#ci.fieldseqn"/>"
															class='editcss' size='5' value='' /></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<input name='custom.fieldremark'
															value="<s:property value="#ci.fieldremark"/>"
															class='editcss' /></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<select name='custom.fieldstate' class='editcss'>
															<option value='1'
																<s:if test="#ci.state==1">selected</s:if>>
																启用
															</option>
															<option value='0'
																<s:if test="#ci.state==0">selected</s:if>>
																停用
															</option>
														</select></div>
													</td>
													<td height="18" bgcolor="#FFFFFF"><div align="center" class="STYLE2 STYLE1">
														<input name='delButton' type='button' value='删除'
															onclick='delrow(this)' class='btn' /></div>
													</td>
												</tr>
											</s:iterator>           
          </tbody>
