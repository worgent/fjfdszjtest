<%@ page contentType="text/html; charset=utf-8" language="java" import="HibernateBeans.cms.cmsClass,java.util.List,util.HibernateUtil,HibernateBeans.cms.attribute" errorPage="" %>
<%
...
td3=tr.insertCell(2);
td3.innerHTML="<input type=\"text\" size=\"3\" id=\"attrvs\" name=\"attrvs\" onclick=\"javascript:getV(this)\" value=\"1\"/><span style=\"color:red\"></span>";
...
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  <tr class="title">
    <td colspan="2" align="center">添加数据类别(步骤2：设定属性)</td>
  </tr>
  <tr>
...
<%=pname%>    </td>
  ...
  <tr>
    <td height="30">名称：</td>
    <td height="30"><input name="name" type="text" id="name" size="50" maxlength="50"  /></td>
  </tr>
  <tr>
    <td height="30">属性：</td>
...
s=HibernateUtil.getSessionFactory().getCurrentSession();
s.beginTransaction();
List attrlist=s.createQuery("from attribute where cid="+pid+" order by sort ASC").list();
s.getTransaction().commit();
...
%>
      <tr>
        <td>
        <input type="text" id="attrname" name="attrname" value="<%=attri.getName()%>"/></td>
        <td>
        ...
        <td><input type="text" size="3" id="attrvs" name="attrvs" onclick="javascript:getV(this)" value="<%=attri.getValueList()%>"/>
<span style="color:red"><%
            ...
            %></span></td>
        <td><input type="text" id="attrsv" name="attrsv" value="<%=attri.getDefaultValue()%>"/></td>
        <td align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><select name="listView" id="listView">
      ...
      <td><select name="addView" id="addView">
      ...
      <td><select name="modView" id="modView">
        ...
        <td><input type='button' value="删除" id="tM<%=i%>" <%if(!attri.getLockme()){%> onClick="javascript:rows(this)"<%}%>/></td>
      </tr>
<%
}
%>
...
this.attr=<%=attrlist.size()%>;
  ...
  <tr>
    <td height="30">&nbsp;</td>
    <td height="30">注：在属性管理中，打*的是必填项目，其他的为选填项。仅当属性类型为单选、多选、列表时才需要填写备选项。后台管理显示可以设定字段是否在自动生成的后台管理页面（列表、新建、修改）中显示</td>
  </tr>
  <tr>
    <td height="30">&nbsp;</td>
    <td height="30"><input type="button" name="button1" id="button1" value="上一步" onclick="javascript:window.history.back()" />&nbsp;&nbsp;<input type="submit" name="button" id="button" value="完成" /></td>
  </tr>
</table>
</form>
</body>
