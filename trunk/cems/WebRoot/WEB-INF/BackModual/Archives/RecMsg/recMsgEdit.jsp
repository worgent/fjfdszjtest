...
<form  method='POST' name='form1' action='/archives/recMsg.do'  id="form1">
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#prec_name").formValidator({onfocus:"最多20个字符",oncorrect:"√"}).inputValidator({min:1,max:20,onerror:"姓名非法,请确认"});
		$("#prec_tel").formValidator({onfocus:"电话号码或小灵通,例059512345678",oncorrect:"√"}).regexValidator({regexp:"^$|^\\d{10,12}$|^(13|15|18)[0-9]{9}$",onerror:"电话号码格式不正确"});
		//$("#prec_mobile").formValidator({onfocus:"手机号",oncorrect:"√"}).regexValidator({regexp:"^$|^(13|15|18)[0-9]{9}$",onerror:"手机号码格式不正确"});
		$("#prec_address").formValidator({tipid:"paddressTip",onfocus:"取件地址",oncorrect:"√"}).inputValidator({min:1,onerror:"取件地址不能为空"});
		$("#prec_post").formValidator({onfocus:"邮编",oncorrect:"√"}).regexValidator({regexp:"zipcode",datatype:"enum",onerror:"邮编格式不正确"});
