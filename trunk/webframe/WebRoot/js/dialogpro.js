...
generic_dialog.prototype.should_use_iframe = ua.ie() < 7
		|| (ua.osx() && ua.firefox());
...
generic_dialog.prototype.focus_first_textbox = function() {
	function focus_textbox(node) {
		var is_textbox = (node.tagName == "INPUT" && node.type.toLowerCase() == "text")
...
generic_dialog.prototype.show_loading = function(loading_html) {
	return this
			.show_dialog('<div id="structs" class="share_composer share_status_post"><div class="loading"><p>'
					+ loading_html + '</p></div></div>');
}
...
generic_dialog.prototype.show_prompt = function(title, content) {
	return this.show_dialog('<h2><span>' + title
			+ '</span></h2><div class="dialog_content">' + content + '</div>');
}
...
generic_dialog.prototype.show_choice = function(title, content, button1,
		button1js, button2, button2js, buttons_left_msg, button3, button3js) {
...
generic_dialog.prototype.show_choice_ajax = function(title, content_src,
		button1, button1js, button2, button2js, buttons_left_msg, button3,
		button3js) {
	this.show_loading('Loading...');
	var handler = function(response) {
		this.show_choice(title, response.getPayload(), button1, button1js,
				button2, button2js, buttons_left_msg, button3, button3js);
	}.bind(this);
	new AsyncRequest().setURI(content_src).setHandler(handler).send();
	return this;
}
generic_dialog.prototype.show_form_ajax = function(title, src, button,
		reload_page_on_success) {
	this.show_loading('Loading...');
	...
	var preSubmitHandler = function(dialog, response) {
		var contents = '<form id="' + form_id + '" onsubmit="return false;">'
				+ response.getPayload() + '</form>';
	...
	var submitHandler = function() {
		new AsyncRequest().setURI(src).setData(serialize_form(ge(form_id)))
				.setHandler(postSubmitHandler)
				.setErrorHandler(postSubmitErrorHandler).send();
	};
	var postSubmitHandler = function(dialog, response) {
		dialog.show_choice(title, response.getPayload(), 'Okay', function() {
					dialog.fade_out(200);
				});
	...
	new AsyncRequest().setURI(src).setReadOnly(true)
...
generic_dialog.prototype.show_form = function(title, content, button, target) {
	content = '<form action="' + target + '" method="post">'
			+ this.content_to_markup(content);
	var post_form_id = ge('post_form_id');
	if (post_form_id) {
		content += '<input type="hidden" name="post_form_id" value="'
				+ post_form_id.value + '" />';
	}
	content += '<div class="dialog_buttons"><input class="inputsubmit" name="confirm" type="submit" value="'
			+ button + '" />';
	content += '<input type="hidden" name="next" value="'
			+ htmlspecialchars(document.location.href) + '"/>';
...
generic_dialog.prototype.content_to_markup = function(content) {
	return (typeof content == 'string') ? '<div class="dialog_body">' + content
			+ '</div>' : '<div class="dialog_summary">' + content.summary
			+ '</div><div class="dialog_body">' + content.body + '</div>';
}
...
generic_dialog.prototype.fade_out = function(interval, timeout) {
	...
	animation(this.obj).duration(timeout ? timeout : 0).checkpoint().to(
			'opacity', 0).hide().duration(interval ? interval : 350)
			.ondone(this.hide.bind(this)).go();
	return this;
}
...
generic_dialog.prototype.build_dialog = function() {
	...
	this.obj.className = 'generic_dialog'
...
pop_dialog.prototype.reset_dialog_obj = function() {
	this.popup.style.top = (document.documentElement.scrollTop
...
contextual_dialog.prototype.reset_dialog_obj = function() {
	...
	if (x < document.body.offsetWidth / 2) {
		...
		var left = Math.min(center, x + this.context.offsetWidth
						- this.arrow_padding_x);
		var arrow = x - left + this.context.offsetWidth + this.arrow_padding_x;
	} else {
		this.arrow.className = 'contextual_arrow';
		var left = Math.max(center, x - this.popup.offsetWidth
						+ this.arrow_padding_x);
		var arrow = x - left - this.arrow_padding_x - this.arrow_width;
	}
	this.popup.style.top = (elementY(this.context) + this.context.offsetHeight
			- this.arrow.offsetHeight + this.arrow_padding_y)
			+ 'px';
...
contextual_dialog.prototype._remove_resize_events = function() {
	if (this._scroll_events) {
		for (var i = 0; i < this._scroll_events.length; i++) {
			removeEventBase(this._scroll_events[i].obj,
					this._scroll_events[i].event, this._scroll_events[i].func);
		}
	}
	this._scroll_events = [];
}
...
copy_properties(ErrorDialog, {
			showAsyncError : function(response) {
				try {
					return (new ErrorDialog()).showError(response
