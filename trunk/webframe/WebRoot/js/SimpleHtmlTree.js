...
function SimpleHtmlTree(trPnlId,mnStl,icnStl,chk){
	...
	this.createTreeNode=function(id,text,type,url,target,icn,clkHdl,ldHdl){
		...
		var pnl=crtDiv();var spcPnl=crtSpn();var mn=crtImg();var sdPnl=crtSpn();var chkBx=crtImg();var icn=crtImg();var lnk=crtAnchr(text);
		...
		pnl.expand=function(){var l=eval(pnl.level);var tn=pnl;for(var i=0;i<l;i++){tn=tn.getPn();tn.fold(true);}}
		...
		pnl.changeChk=function(chk){pnl.chk=chk;pnl.getChkBx().src="../images/chk/"+chk.toLowerCase()+".gif";}
		pnl.simpleCheck=function(){var chk=pnl.chk;if(chk=="unchk"){pnl.changeChk("chk");}else{pnl.changeChk("unchk");}}
		pnl.flexCheck=function(){
			var chk=pnl.chk;
			if(chk=="unchk"){pnl.changeChk("chk");chkChild(pnl,"chk");chkParent(pnl);
			}else if(chk=="chk"||chk=="chkds"){pnl.changeChk("unchk");chkChild(pnl,"unchk");chkParent(pnl);}
			function chkChild(tn,status){
				var childs=tn.getChild();
				var count=childs.length;var tmp;
				for(var i=0;i<count;i++){tmp=childs[i];if(tmp.chk!=status){tmp.changeChk(status);}chkChild(tmp,status);}
			}
			function chkParent(tn){
				...
				for(var i=level-1;i>=0;i--){
					...
					for(j=0;j<childsCount;j++){
						status=childs[j].chk;
						if(status=="chk"){chkCount++;}else if(status=="chkds"){halfChkCount++;}}
					if(chkCount==childsCount){tmp.changeChk("chk");
					}else if(chkCount>0||halfChkCount>0){/*or (checkNodeCount+halfCheckNodeCount)>0*/tmp.changeChk("chkds");
					}else if((chkCount+halfChkCount)==0){tmp.changeChk("unchk");}
				}
			}
		}
		/*init*/
	...
	var mdClr=function(nd,txtColor,bgColor){var link=nd.getLnk();var stl=link.style;stl.color=txtColor;stl.backgroundColor=bgColor;}
	...
	this.getChecked=function(){
		...
		var nds=new Array();var n=0;getCheckedNodes(ROOT);return nds;
		function getCheckedNodes(node){
			var count;var nodes=node.getChild();count=nodes.length;
			for(var i=0;i<count;i++){var childNode=nodes[i];if(childNode.chk=="chk"){nds[n]=childNode;n++;}getCheckedNodes(childNode);}
		}
	}
