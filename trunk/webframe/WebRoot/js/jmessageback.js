...
(function($) {
	...
	$.megaboxx = {
		// var
		// megaboxx=function(endpoint){if(endpoint){this.endpoint=endpoint;}else{this.endpoint='/message/ajax.do';}}
		/*
		 * STATUS_ALL=0; STATUS_READ=1; STATUS_UNREAD=2; STATUS_NONE=-1;
		 */
		/*
		 * if(typeof tokenizer!='undefined'){
		 * tokenizer.prototype.onselect=function() { megaboxx.reset_rand_id(); } }
		 */
		...
		status_menu_onclick : function(obj, action, threads) {
			...
			if (action == 'delete') {
				if (typeof obj == 'boolean' && obj) {
					...
					for (var i = 0; i < threads.length; i++) {
						var row = document.getElementById('thread_'
								+ threads[i]);
					...
					if (!loading) {
						...
						loading.innerHTML = [
								'<table id="loading_boxx" class="',
								boxx.className, '"><tr><td></td></tr></table>']
								.join('');
						...
						window.setTimeout("ge('loading_boxx').style.display='none'",1000);
							...
							confirm( ((threads.length == 1)?'你确定要删除这封站内信吗？': '你确定要删除这些站内信吗？'),function(){
									//this.status_menu_onclick(true,action,threads);
														...
														for (var i = 0; i < threads.length; i++) {
															...
															//var row=$('thread_'+ threads[i]);
															//row.parentNode.removeChild(row);
															
															alert(threads[0]);							
															$("tr[id='thread_"+threads[i]+"']").remove();
															
															//tr_id = $("#megaboxx>tbody>tr:last").attr("thread_"+threads[i]);    
															//$('#'+tr_id).remove(); 
														}
					...
					confirm("你确定要删除这封站内信吗?",this.status_menu_onclick(true, action,threads));
					var dialog = new contextual_dialog();
					dialog.set_context(obj);
					dialog.show_choice(((threads.length == 1)
									? '你确定要删除这封站内信吗？'
									: '你确定要删除这些站内信吗？'), '', '删除', function() {
								this[0].status_menu_onclick(true, action,
										threads);
								this[1].hide()
							}.bind([this, dialog]), '取消', function() {
				...
				for (var i = 0; i < threads.length; i++) {
					...
					if (action == 'mark_read') {
						row.className = trim(row.className.replace(
								'new_message', ''));
					} else {
						row.className = trim(row.className.replace(
			...
			$.ajax({
			    ...
			    dataType: 'html',
			    timeout: 1000,
			    error:this.ajax_callback(this),  
			    success: this.ajax_callback(this)
			});
		...
		ajax_callback : function(response) {
			...
			if (count > 0) {
				message_count.innerHTML = '站内信<span class="count">(' + count
						+ ')</span>'
			} else
				message_count.innerHTML = '站内信';
			var top = ge("message_count_top");
			if (count > 0) {
				if (top.innerHTML.indexOf('/') > 0) {
					new_count = count
							+ '封未读'
							+ top.innerHTML.substr(top.innerHTML.indexOf('/'),
									top.innerHTML.length);

				} else {
					new_count = count + '封未读/' + top.innerHTML;
				}
			} else
				new_count = top.innerHTML.substr(
						top.innerHTML.indexOf('/') + 1, top.innerHTML.length);
...
/*
function XN.element.extend=function(_10c){
