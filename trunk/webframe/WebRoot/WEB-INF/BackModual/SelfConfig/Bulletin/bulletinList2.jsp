...
<script type="text/javascript">
		...
		jConfirm('您确定删除该狩猎信息!', 'Confirmation Dialog', function(r) {	
    		if (r){
				...
				try{
					...
					if (null != root){
						...
						if(0<rowSet.item(0).selectSingleNode("value").text){
						   window.location.href='<%=path%>/selfconfig/bulletin.do?action=list&search.ptype=2';
						}
						else{
						    jAlert('删除失败!');  
						}
	...
	<body>
	  ...
	  <div>
		...
		<table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0">
		    ...
		    <a href='<%=path%>/selfconfig/bulletin.do?action=add&search.ptype=2' target='_self'>
			增加
			</a>
			<tr>
				 <s:label value="狩猎信息" ></s:label>
			</tr>
			<tr>
				<td>
			...
			<tbody>
				<s:iterator id="bulletin" value="%{pageList.objectList}">
					<tr>
						<td class="bgColor3">
							 <span class="font1">
							 <a href='<%=path%>/selfconfig/bulletin.do?action=view&search.ptype=2&search.pid=<s:property value="#bulletin.ID" />' target='_self'>
						...
						<td class="bgColor4">
							...
							|
							<a href='<%=path%>/selfconfig/bulletin.do?action=edit&search.ptype=2&search.pid=<s:property value="#bulletin.ID" />' target='_self'>
                                修改
                            </a>
                            |
