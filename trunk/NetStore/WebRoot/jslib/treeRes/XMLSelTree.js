...
var m_oConfig ;
var m_bCheckChangeChildByFather;//??????checkbox??????????????????????????????????false ????true ??
...
/************************************************
** InitTree()
p_sXMLFile??xml????????????????m_sXMLFile	= "TreeNode.xml";
p_sXSLPath??xsl??????????m_sXSLPath	= "../../treeRes/";
p_oSrcDiv??div????id??<DIV id="SrcDiv" onselectstart="selectstart()"></DIV>
p_bCheckChangeChildByFather check?????????????????? ??????????????????????false ????true ??
p_bAutoGoFisrtLeaf????????????????????????
p_bChangeLeafImg??????????????????????????
p_isExpand ??????????????
************************************************/
function InitTree(p_sXMLFile, p_sXSLPath, p_oSrcDiv,p_bCheckChangeChildByFather,
					p_bAutoGoFisrtLeaf, p_bChangeLeafImg , p_isExpand  )
{
	...
	m_bCheckChangeChildByFather = p_bCheckChangeChildByFather;
...
//alert("val="+m_bCheckChangeChildByFather);
	if (!m_bCheckChangeChildByFather){
		var b = false;
		if (bChecked)
		{
...
{    
	...
	c = m_oConfig.oSrcDiv.children.length;
	for (i=0;i< c ;i++ )
	{
		RecursionTree(m_oConfig.oSrcDiv.children[i]);    
	}
}
/*
