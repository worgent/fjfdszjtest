...
document.forms[0].action="indexFirst.do?status=specialPrice&page="+document.getElementById("pageId").value+"&totalPages="+document.getElementById("totalPages").value; 
document.forms[0].submit();
}
</script>

</head>
<body class="ProducTypeHome3">
...
<logic:iterate id="item" name="ppage" property="resultList" scope="request">
  <tr>
  <td align="center"  bgcolor="#ffffff" height="130">
 ...
 <td><a href=""><img src="images/shoucang1.gif" border="0"></img></a></td>
 </tr>
 </table>
...
<table class="tbl" width="98%"  cellspacing="0" cellpadding="0"  >
	...
	<td class="main" align="left">
								...
								%>
								<a
									href="indexFirst.do?status=specialPrice&targetPage=1"
									class="pagenav">首页</a>
								<%
								if (p.isHasPrevious()) {
								%>
								<a
									href="indexFirst.do?status=specialPrice&targetPage=<%=currentPage == 1 ? 1 : currentPage - 1%>"
									class="pagenav">上一页</a>
								<%
								} else {
								%>
								...
								%>
								<a
									href="indexFirst.do?status=specialPrice&targetPage=<%=currentPage == p.getTotalPages() ? p.getTotalPages() : currentPage + 1%>"
									class="pagenav">下一页</a>
								<%
								} else {
								%>
								...
								%>
								<a
									href="indexFirst.do?status=specialPrice&targetPage=<%=p.getTotalPages()%>"
   ...
   <input type="button" value="Go" onclick="setAction('qry')"  onkeypress="number()" class="button"/>
