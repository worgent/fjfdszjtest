...
document.forms[0].action="indexFirst.do?status=moreNewProduct&page="+document.getElementById("pageId").value+"&totalPages="+document.getElementById("totalPages").value; 
...
<tr><td colspan="3" style="color:#ff7000">最新上架</td></tr>
...
<table class="tbl" width="98%"  cellspacing="0" cellpadding="0"  >
	...
	<td class="main" align="left">
								...
								%>
								<a
									href="indexFirst.do?status=moreNewProduct&targetPage=1"
									class="pagenav">首页</a>
								<%
								if (p.isHasPrevious()) {
								%>
								<a
									href="indexFirst.do?status=moreNewProduct&targetPage=<%=currentPage == 1 ? 1 : currentPage - 1%>"
									class="pagenav">上一页</a>
								<%
								} else {
								%>
								...
								%>
								<a
									href="indexFirst.do?status=moreNewProduct&targetPage=<%=currentPage == p.getTotalPages() ? p.getTotalPages() : currentPage + 1%>"
									class="pagenav">下一页</a>
								<%
								} else {
								%>
								...
								%>
								<a
									href="indexFirst.do?status=moreNewProduct&targetPage=<%=p.getTotalPages()%>"
