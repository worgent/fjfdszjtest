...
ChangePasswordWindow=Ext.extend(Ext.Window,{
 ...
 userconfigWindow=Ext.extend(Ext.Window,{
	...
	save:function() {
			var url = '/system/manage/userConfig.shtml?search.leftMargin='+this.fp.form.findField("leftMargin").getValue()+'&search.topMargin='+this.fp.form.findField("topMargin").getValue()+'&search.sendCode='+this.fp.form.findField("sendCode").getValue();
			try{
				var oXMLDom	= new ActiveXObject("Msxml.DOMDocument");
				oXMLDom.async = false ;
				oXMLDom.load(url); 
		        ...
		        items:[{
	                ...
	                items:[{
			        	...
			        	buttons: [{
				            ...
				            width:50,
				            handler: function(){
				            	document.all.ClientMsgIframe.src="/netprint/clientmsg/sendclientmsg.shtml?actionType=exsearch&search.pname="+document.all.clientname.value+"&search.pbill_sort=1";
				            }
				        },{
				            text: '确定',
				            width:50,
