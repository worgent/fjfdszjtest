...
ChangePasswordWindow=Ext.extend(Ext.Window,{
 ...
 /*用户信息配置start*/
 userconfigWindow=Ext.extend(Ext.Window,{
 	title : '用户配置',		
	width : 300,		
	iconCls:"icon-password",
	height : 245,	
	...
	createFormPanel :function() {
		return new Ext.form.FormPanel({
			...
			items : [{
			        width:270,
					xtype:'panel',
					html:'<font color=red>注意：<br>以下单位是毫米，负值向左(上)，正值向右(下)。</font>'
				},{
					name : 'leftMargin',
					fieldLabel : '单张打印左边距',
					xtype:'numberfield',
					...
					listeners: {'focus': function(obj,e) {
					    //alert(Ext.getCmp('sendCode').value);
						getsendcode(obj,Ext.getCmp('sendCode'));
					}}
				}]
		});
	},					
	save:function() {
			var url = '/system/manage/userConfig.shtml?search.leftMargin='+this.fp.form.findField("leftMargin").getValue()+'&search.topMargin='+this.fp.form.findField("topMargin").getValue()+'&search.leftMarginex='+this.fp.form.findField("leftMarginex").getValue()+'&search.topMarginex='+this.fp.form.findField("topMarginex").getValue()+'&search.sendCode='+this.fp.form.findField("sendCode").getValue();
			try{
				var oXMLDom	= new ActiveXObject("Msxml.DOMDocument");
				oXMLDom.async = false ;
				oXMLDom.load(url); 
				...
				if (null != root){
					var rowSet = root.selectNodes("//Result");
					if (1 == rowSet.item(0).selectSingleNode("errCode").text){
						Ext.Msg.alert("提示","用户信息配置成功,请重新登录!",function(){
						//Ext.getCmp('leftMargin').setValue(100);
						//Ext.getCmp('topMargin').setValue(100);
						this.close();
						logout();
						},this);
					}else{
						Ext.Msg.alert("提示", rowSet.item(0).selectSingleNode("errMsg").text,function(){
							form.findField("leftMargin").focus(true);
						},this);
					}
				}
			}catch(e){ 
  ...
  //2009-10-28
 function getsendcode(obj,otherobj){
 		/*监听事件开始*/		
		        ...
		        items:[{
	                ...
	                items:[{
			        	...
			        	buttons: [{
				            ...
				            width:50,
				            handler: function(){
				            	document.all.ClientMsgIframe.src="/netprint/clientmsg/sendclientmsg.shtml?search.action=exsearch&search.pname="+document.all.clientname.value+"&search.punit="+document.all.punit.value+"&search.ptel="+document.all.ptel.value+"&search.pbill_sort=1";
				            }
				        },{
				            text: '确定',
				            width:50,
				            handler: function(){
				            	...
				            	//当设置的客户编辑与原来相同时删除
				            	if(otherobj.getValue()==document.ClientMsgIframe.frmid.value)
				            	{
				            		//取得客户姓名
									obj.setValue(" ");
									//内部id的保存
					            	...
					            	//取得客户姓名
									obj.setValue(document.ClientMsgIframe.frmname.value);
									//内部id的保存
									otherobj.setValue(document.ClientMsgIframe.frmid.value);
								}
				            	searchPlan.close();
				            }
				        }]
...
/*用户配置信息修改调用*/
function userconfig(){
	var userconfigWin=new userconfigWindow();
	Ext.get("desktop").mask();
	userconfigWin.show();	
}
/********暂时不知道功能如何使用
