...
<tt:TextField name="search.psendpost" value="search.sendpost" width="93" height="16" cssClass="check" verify='int+' shade='true'  required='true' requiredColor='#ffffff'/>
  </div>
...
<tt:TextField name="search.psendsign" value="search.sendsign" width="250" cssClass="check" verify='string'  shade='true' requiredColor='#ffffff'/>
  <div id="Layer44">
<input type="checkbox" name="search.psendinsure" id="psendinsure" value="1" <ww:if test="\"1\".equals(search.sendinsure)">checked</ww:if>  /> <label for="psendinsure">保价</label> 
  </div>
  <div id="Layer45"><label for="psendinsuremoney">金额</label> 
<tt:TextField name="search.psendinsuremoney" value="search.sendinsuremoney" width="129" cssClass="check" verify='int'  required='true' shade='true' requiredColor='#ffffff' />	  
  </div>
  </div>
	...
	var saveButton =new Ext.Button({
        ...
        handler: function(){
        	if (fm2.isValid()) {
        	    ...
        	    if(Ext.get('search.psendinsuremoney').dom.value>99999){
	        	    Ext.Msg.show({
	 					title:'信息',
						msg: '保额不能大于99999!',
						modal : true,
		        ...
		        items:[{
	                ...
	                items:[{
			        	...
			        	buttons: [{
				            ...
				            handler: function(){
				                if(sort==0)//收
				            	document.all.ClientMsgIframe.src="/netprint/clientmsg/recclientmsg.shtml?search.action=exsearch&search.pname="+document.all.clientname.value+"&search.punit="+document.all.punit.value+"&search.ptel="+document.all.ptel.value+"&search.pbill_sort="+sort;
				            	else if (sort==1)//寄
				            	document.all.ClientMsgIframe.src="/netprint/clientmsg/sendclientmsg.shtml?search.action=exsearch&search.pname="+document.all.clientname.value+"&search.punit="+document.all.punit.value+"&search.ptel="+document.all.ptel.value+"&search.pbill_sort="+sort;
	                    ...
	                    items:[{
	                    	width:"100%",
	                    	height:"100%",
	                        html: "<iframe id='ClientMsgIframe' name='ClientMsgIframe' src=''width='100%' height='250' frameborder=0/>"
	                    }]
			        }]
		        }]
		    });
				        ...
				        items:[{
			                ...
			                items:[{
					        	...
					        	buttons:[{
					                ...
					                baseCls:'x-plain',
					        		xtype: 'panel',
						            html:'<font color=red>提示信息:<br>正确格式:姓名，手机，电话，地址，邮编,<br>例如:贾大庆 ，15235021186 ， 0595XXXX，山西省 忻州市 忻府区 和平西街国力花园4号楼4单元4楼西门 ，034000</font>',
						            width:350  // 设置宽度，百分比的需加‘号
					        }
					        ]
					     }]
				    });
