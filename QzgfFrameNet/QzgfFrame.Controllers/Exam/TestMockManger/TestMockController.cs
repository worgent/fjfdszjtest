...
{
    public class TestMockController : BaseController
    {
        ...
        private TestMockFacadeEx testmockFacadeEx { set; get; }
        ...
        {
            TestMock result = new TestMock();
            ExamTestMock ExamTestMock = null;
            IList<ExamTestMock> etms = testmockFacade.LoadAll("ID", "TestID='" + testid + "'");
            if (etms.Count == 0)       //模拟考卷表不存在相同的试卷ID
            {
                ExamTestMock = new ExamTestMock();
                ExamTestMock.TestID = testid;
                ExamTestMock.ID = "0";
            }
            else
            {
            ...
            ExamTestMock.CreatorID = this.currentUser.UserInfo.Id;
            result.ExamTestMock = ExamTestMock;
            ...
            IList<ExamExamineeRange> ExamineeRanges = examineerangeFacade.LoadAll("ID", "TestSetID='" + ExamTestMock.ID + "'");
        ...
        {
            TestMock result = null;
            result = testmockFacade.GetTestMock(id);
            ExamTestContent ExamTestContent = testFacade.Get(result.ExamTestMock.TestID);
            ExamCourseType ExamCourseType = courseFacade.Get(ExamTestContent.TypeID);
            result.TestName = ExamTestContent.Name;
            ViewBag.testname = ExamTestContent.Name;
            ViewBag.typeid = ExamCourseType.Id;
        ...
        //FormCollection
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Edit(TestMock entity)
        {
            try
            {
                bool result = false;
                ...
                if (entity.ExamTestMock.ID == null || entity.ExamTestMock.ID == "0")
                    result = this.testmockFacadeEx.Save(entity);
                else
                    result = this.testmockFacadeEx.Update(entity);
                if (result) msg = "操作成功";
        ...
        public ActionResult Delete(string id)
        {
            bool result = false;
            string msg = "操作失败";
            result = testmockFacadeEx.Delete(id.ToString());
            if (result) msg = "操作成功";
            return Json(new { Type = result, Message = msg }, JsonRequestBehavior.AllowGet);
        }
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Get)]
        public ActionResult Mock(string id)
        {
            //浏览时返回具体值
            ...
            string sql = "";
            sql = " TestId = '" + id + "'";
            IList<ExamTestMock> examTestMock = testmockFacade.LoadAll("Id", sql);
            ViewData["hbnTimeLength"] = Convert.ToInt32(examTestMock[0].TimeLength) * 60;
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Mock(TestContent entity)
        {
            bool bools = false;
            string msg = "";
            ...
            //提交试卷后，将该"模拟考试"试卷的“IsDelete”修改为“false”，为不可删除状态
            ExamTestMock examTestMock = new ExamTestMock();
            examTestMock = testmockFacade.Get(entity.ExamTestMock.ID);
            if (examTestMock != null)
            {
                examTestMock.IsDelete = false;
                testmockFacade.Update(examTestMock);
            }
        ...
        public override string GridPager(int page, int pagesize, string sortname, string sortorder, string gridviewname, string gridsearch)
        {
            ...
            return testmockFacade.FindByPage(page, pagesize, userID, Role);
        }
    }
}
