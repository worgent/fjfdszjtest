...
namespace QzgfFrame.Controllers.Archives.NatureManger
{
    public class NatureController : BaseController
    {
        ...
        {
            ArchiveNature result = natureFacade.Get(id);
            return View("Edit", result);
        }
        //
        ...
        //FormCollection
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Edit(string id, ArchiveNature entity)
        {
            try
            {
                bool result = false;
                string msg = "操作失败";
                if (id == "0")
                    result = this.natureFacade.Save(entity);
                else
                    result = this.natureFacade.Update(entity);
                if (result) msg = "操作成功";
        ...
        public ActionResult GetAll()
        {
            ...
            foreach (ArchiveNature item in natures)
            {
                mapList.Add(new
                {
                    natureid = item.Id,
        ...
        public ActionResult Delete(string id)
        {
            ...
            result = natureFacade.Delete(id.ToString(),out DelFlag);
            if (result) msg = "操作成功";
        ...
        {
            return natureFacade.FindByPage(page, pagesize);
        }
    }
}
