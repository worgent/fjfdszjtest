...
$(function () {
    ...
    var v = $("form").validate({
        ...
        submitHandler: function () {
            ...
            var strdata = $("#LicensePlateNumber").val();
            $.getJSON('/Resources/Vehicle/GetVehicleNo/?id=' + encodeURI(strdata), { Rnd: Math.random() }, function (data) {
                if (data.Type) {
                    if ($("#Id").val() != data.Id) {
                        $("#showTerim").html("该车牌号在车辆信息中已存在，不可重复添加！");
    ...
    getid.push("");
    getid.push("");
    $.getJSON('/Resources/Personnel/GetDropDownList/?id=' + getid, { Rnd: Math.random() }, function (data) {
...
/*判断在车辆信息中该车牌号是否存在*/
function getVehicleNo(val) {
    var arrData = new Array();
    var getFlag = false;
    if (val != "") {
        $.getJSON('/Resources/Vehicle/GetVehicleNo/' + encodeURI(val), { Rnd: Math.random() }, function (data) {
            if (data.Type) {
                if ($("#Id").val() != data.Id) {
                    $("#showTerim").html("该车牌号在车辆信息中已存在，不可重复添加！");
                    getFlag = false;
    ...
    fm.ajaxSubmit(function (result, success, form) {
        if (result.Type) {
            ...
            $.ligerDialog.confirm("是否继续添加记录?", '保存成功!', function (ok) {
                if (ok) {
                    window.location.href = "/Resources/Vehicle/add/?id=0&frameid=" + $("#frameid").val();
                }
                else
                    window.location.href = "/Resources/Vehicle?_menuid=" + $("#frameid").val();
            });
        }
        else {
            $.ligerDialog.closeWaitting();
            $.ligerDialog.error("操作失败！", function () {
            });
        }
    });
}
