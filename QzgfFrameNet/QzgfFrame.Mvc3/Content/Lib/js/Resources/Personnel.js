...
$(function () {
    ...
    var v = $("form").validate({
        ...
        submitHandler: function () {
            ...
            var strdata = $("#IDCardNumber").val();
            $.getJSON('/Resources/Personnel/getIdCardNo/?id=' + strdata, { Rnd: Math.random() }, function (data) {
                if (data.Type) {
                    if ($("#Id").val() != data.Id) {
    ...
    fm.ajaxSubmit(function (result, success, form) {
        if (result.Type) {
            ...
            $.ligerDialog.confirm("是否继续添加记录?", '保存成功!', function (ok) {
                if (ok) {
                    window.location.href = "/Resources/Personnel/add/?id=0&frameid=" + $("#frameid").val();
                }
                else
                    window.location.href = "/Resources/Personnel?_menuid=" + $("#frameid").val();
            });
        }
        else {
            $.ligerDialog.closeWaitting();
...
function getIdCardNo(val) {
    ...
    var getFlag = false;
    if (val != "") {
        $.getJSON('/Resources/Personnel/getIdCardNo/' + val, { Rnd: Math.random() }, function (data) {
            if (data.Type) {
                if ($("#Id").val() != data.Id) {
                    $("#showTerim").html("该身份证号在人员信息中已存在，不可重复添加！");
                    $("#IDCardNumber").focus();
