...
function AddSuppliesRow(SuppliesType) { //读取最后一行的行号，存放在txtTRLastIndex文本框中
    ...
    //添加列:所属区县
    var newDistrictTD = newTR.insertCell(1); newDistrictTD.className = "sno";
    newDistrictTD.innerHTML = "<input type='hidden' name='DistrictId" + rowID + "' id='DistrictId" + rowID + "' value='" + SuppliesType.DistrictId.toString() + "'/><input name='DistrictName" + rowID + "' value='" + SuppliesType.DistrictName.toString() + "' id='DistrictName" + rowID + "' type='text' size='20'  class = 'disable-dd-text'  readonly='readonly' />";
...
function DelSuppliesRow(rowid) {
    ...
    $("#Count").val(Count)
    var DistrictId = signFrame.rows[rowIndex].cells[1].getElementsByTagName("input")[0].value;
    ...
    for (var k = 0; k < gosArray.length; k++) {
        ...
        if (goObject.did == DistrictId &&  goObject.tid == SuppliesTypeId) {
...
function GetSuppliesRow() {
    ...
    ary.length = 0;
    for (i = 1; i < rowscount; i++) {
        var DistrictId = signFrame.rows[i].cells[1].getElementsByTagName("input")[0].value;
        var DistrictName = signFrame.rows[i].cells[1].getElementsByTagName("input")[1].value;
        ...
        strobj["DistrictName"] = DistrictName;
...
function GetOrgIds() {
    ...
    for (var j = 0; j < orgids.length; j++) {
        var orgId = new Object(); orgId = orgids[j];
        var delFlag = true;
        for (i = 1; i < rowscount; i++) {
            var DistrictId = signFrame.rows[i].cells[1].getElementsByTagName("input")[0].value;
            if (orgId["DistrictId"] == DistrictId ) {
                delFlag = false;
            }
        }
        if (delFlag) {
    ...
    for (var k = 0; k < ary.length; k++) {
        var orgId = new Object(); orgId = ary[k];
        orgids.remove(orgId);
    }
    return orgids;
}
