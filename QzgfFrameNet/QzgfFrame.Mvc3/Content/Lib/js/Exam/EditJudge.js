function f_checkData() {
    ...
    for (i = 1; i < subjectFrame.rows.length; i++) {    
        if (subjectFrame.rows[i].cells[1].getElementsByTagName("input")[0].value == "") {
            subjectFrame.rows[i].cells[1].getElementsByTagName("input")[0].focus();
            return false;
        }
    }
    return true;
...
function f_save() {
    $("#judges").val(JSON2.stringify(GetJudgeRow()));
        ...
        if (result.Type) {
            ...
            if ($("#state").val() == "add") {
                $.ligerDialog.confirm("是否继续添加记录?", '保存成功!', function (ok) {
                    if (ok) {
                        window.location.href = "/Exam/Subject/EditJudge?id=0";
                    }
                    else
