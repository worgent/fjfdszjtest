...
function AddIODetailRow(EquipModel) { //读取最后一行的行号，存放在txtTRLastIndex文本框中
    ...
    newUnitTD.className = "sno";
    var str = "";
    str += "<select name='UnitName" + rowID + "' id='UnitName" + rowID + "' ltype = 'select' >";
    for (var k = 0; k < UnitList.length; k++) {
        str += "<option value='" + UnitList[k].UnitName + "'>" + UnitList[k].UnitName + "</option>";
    }
    str += "</select>";
    newUnitTD.innerHTML = str;

    //添加列:设备状态
    var newStateTD = newTR.insertCell(5);
    var str = "";
    str += "<select name='EquipState" + rowID + "' id='EquipState" + rowID + "' >";
    str += "<option value='0'>正常</option>";
    str += "<option value='1'>坏件</option>";
    str += "<option value='2'>报废</option>";
    str += "</select>";
    newStateTD.innerHTML = str;
    
    //添加列: 数量
    var newNumTD = newTR.insertCell(6);
    newNumTD.innerHTML = "<input type='text' name='Num" + rowID + "' id='Num" + rowID + "'  size='4'  style='width:50px;' maxlength='4'  class = 'dd-text' onkeyup='GetCount(" + rowID + ")' validate = '{required:true,isNumCode:true}'/>";
...
function GetIODetailRow() {
    ...
    for (i = 1; i < rowscount; i++) {
        ...
        var UnitName = signFrame.rows[i].cells[4].getElementsByTagName("select")[0].value;
        var EquipState = signFrame.rows[i].cells[5].getElementsByTagName("select")[0].value;
