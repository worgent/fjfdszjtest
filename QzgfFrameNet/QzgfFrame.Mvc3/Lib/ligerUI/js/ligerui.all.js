...
* iframe.attr("id", framename);//2011-08-24 ie不支持name,仅支持id
* 修改时间:2011.10.28
* 增加功能:
* 2.修改ligerTree,修改结点提交信息整型为字符串,不进行转化.
parseInt(treeitem.attr("treedataindex"))
* 3.getCheckedAndIncomplete()增加得到选中和部分选中的结点信息

* 4.addRowData(data).增加带模块行数据json结构.
* 修改时间:2011.12.21
* 5.ligerTab.js.增加id,
原因是ie和firefox的内核是不一样的，ie是用过id来生成对象，可是firefox是通过name来生成对象。
 $("iframe", contentitem).attr("name", tabid);
* 6.ligerDialog.js按键异常
 g.dialog.close = function() 
 增加  $('body').unbind('keydown.dialog');
* 修改时间:2012.01.17
* 7.针对树型结构，在onselect事件中增加层级数据
p.onSelect && p.onSelect({data:treenodedata,target:treeitem[0]});
修改为
var currentLevel = parseInt(treeitem.attr("outlinelevel"));
p.onSelect && p.onSelect({data:treenodedata,level:currentLevel,target:treeitem[0]});
针对下拉框的树还需修改
在方法tree.onSelect = function(node)中增加支持用户自定义方法
 p.onSelect && p.onSelect(node);
* 8.选项卡双击关闭。
*g.tab.menu && po.addTabItemContextMenuEven(tabitem);
增加
tabitem.dblclick(function()
                    {
                         if($(".l-tab-links-item-close", tabitem).hasClass(".l-tab-links-item-close")){
...
(function($) {
    ...
    $.fn.ligerComboBox = function(options) { 
        ...
        if (p.tree)
          ...
          $("table tr td", g.selectBox).removeClass("l-selected");
         	$("table tr td[value=" + value + "] ", g.selectBox).addClass("l-selected");
            }
        },
        selectText: function (text)
        {
         	var value = g.findValueByText(text);
            po.changeValue(value, text);
          $("table tr td", g.selectBox).removeClass("l-selected");
         	$("table tr td[value=" + value + "] ", g.selectBox).addClass("l-selected");
        },   
        /**更新动态更改下拉框树值 update by cpe 20120116**/
...
//                    }
                    ...
                    {  
                       ...
                       // alert(column.type);
                            if (column.type && column.type == "date")
                            {
                             ...
                             //update by cpe 20111019
                            else if (column.type && column.type == "bool")
                            {          
                                var booldata = item[this.columnname];
                                if(column.func)
                                  content=f_convertData(booldata,column.func);
                            } 
                            else if (column.type && column.type == "download")
                            {          
                                var booldata = item[this.columnname];
                                var strdata=column.linkName;
                                content="<a href='"+column.link+booldata+"'>"+strdata+"</a>";
                            } 
                            //update by hld 20120116
                             else if (column.type && column.type == "javascript")
                            {          
                                var keyArray= new Array();
                                var valueAray= new Array();
                                keyArray=this.columnname.split(',');
                                for(i=0;i<keyArray.length;i++)
                                {
                                    valueAray.push(item[keyArray[i]])
                                }                                 
                                var func=column.func +'(';
                                for(i=0;i<valueAray.length;i++)
                                {
                                    func+='"'+ valueAray[i] +'"';
                                    if(i<valueAray.length-1)
                                    {
                                       func +=',';
                                    }
                                }
                                func +=')';
                                content="<a href='javascript:;' onclick='"+ func +"'>"+column.linkName+"</a>";
                            } 
                             //update by cpe 20110930
                            else if (column.type && column.type == "object")
                            {
                             	 var strObject = item[this.columnname];
                                 $(strObject).each(function (j, ite) {
                                  	var colname=column.value  ;
                                  	if(strObject .hasOwnProperty (colname))
                                  	{ 
                                  		//alert(this[colname]);
                                  		//alert(this['id']);
                                  		content = this[colname];
                                  	}
                                  	});
                            }
                            else
                            {
                              var strdata=item[this.columnname];
                              if(strdata==null)
                                 content="";
                              else
            ...
            { 
                ...
                //复制第一行数据
                if(rowindex>1) {
                    rowdata=g.currentData[p.root][0];
                }
                ...
                { 
                    ...
                    if (columnname)
                    {
                        rowdata[columnname] = curdata[columnname];
                        if (column.type && column.type == 'int') rowdata[columnname] = 0;
    ...
    $.fn.ligerTree = function (p)
    {
        this.each(function ()
        {
            ...
            var g = {
                ...
                getCheckedAndIncomplete: function ()
                {
                    //$(obj).hasClass("l-checkbox-incomplete")
                    if (!p.checkbox) return null;
                    var nodes = [];
                    $(".l-checkbox-checked", g.tree).parent().parent("li").each(function ()
                    {
        ...
        selectNode: function (selectNodeParm)
        {
            ...
            $("li", g.tree).each(function ()
            {
                ...
                var treedataindex = parseInt(treeitem.attr("treedataindex"));
                var treenodedata = po.getDataNodeByTreeDataIndex(g.data, treedataindex);
                if (treedataindex.toString() == selectNodeParm.toString())
                {
                    g.selectNode(this);
                }
            });
            ...
            var po = { 
                ...
                {  
                    ...
                    for (var i = 0; i < data.length; i++)
                    {
                        ...
                        treehtmlarr.push('"');  
                        treehtmlarr.push('>');
                        /**更新树型，当初始化时，选择框值及样式update by cpe  20120116**/
                        if(data[i].ischecked)
                        ...
                        //by cpe 
                        if(data[i].ischecked)
                          p.onSelect && p.onSelect({data:data[i],target:null});
                    }
                    treehtmlarr.push("</ul>"); 
                    return treehtmlarr;
                ...
                setTreeEven : function()
                {
                    p.onContextmenu && g.tree.bind("contextmenu",function(e){ 
                        ...
                        if(!treeitem) return;
                        //var treedataindex = parseInt(treeitem.attr("treedataindex"));
                        var treedataindex = treeitem.attr("treedataindex");
                        var treenodedata = po.getDataNodeByTreeDataIndex(g.data,treedataindex); 
