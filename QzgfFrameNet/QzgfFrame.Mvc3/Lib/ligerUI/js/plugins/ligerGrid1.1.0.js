        ...
        var g = {
            ...
            updateData: function(cell, value, rowObj)
            {
              //debugger;
                ...
                var rowindex = row.attr("rowindex");
                //2011.12.4修改数据，支持表格树型信息 fjfdszj
                /*
                var rowdata = g.currentData[p.root];//非树型
                var treelevel =parseInt(row.attr("treelevel"));
                var tree = p.tree;
                while(--treelevel)
                {
                    rowdata=rowdata[tree.childrenName];
                }
                rowdata = rowdata[rowindex];
                */
                var rowid = row.attr("rowid");
                var rowdata = g.records[rowid];
                //===================
            ...
            toggleCol: function(columnparm, visible)
            {
                ...
                {
                    headercell.hide();
                    var testbody = $(".l-grid-row-cell[columnindex='" + columnindex + "']", g.gridbody);
                    testbody.hide();
