...
//      {label:"预计采购单价(元)",name:"pre_buy_price",hide:"all",id:"pre_buy_price",listeners:{"blur":clcPrice},allowBlank:false,xtype:"number"},
 ...
 function getBuyOrderTbar(){
     return [
        ...
        {text:"申请审核",iconCls:"orderCheck",
         handler:function(){
              var d=getData("app_sg_buy_orderList");
              if(!d.buy_id){
                  $alt("请选择一个采购单进行申请！");
                  return;
              }
         ...
         {text:"取消采购单",iconCls:"gridItemDel",
          handler:function(){
              var d=getData("app_sg_buy_orderList",["buy_id"]);
              if(!d.buy_id){
                  $alt("请选择一个采购单进行取消！");
                  return;
              }
              ...
              doPost("/buy_order_info.modify.go",d,function(){
                 load("app_sg_buy_orderList",d);
                 load("app_sg_buy_orderList2",{});
              });
              
          }},"-",
          {text:"刷新",iconCls:"gridBarItemQuery",handler:cLoadEvent("app_sg_buy_orderList")}    
     ];
 }  
 ...
 function getBuyOrderDetailTbar(){
     ...
     var c=function(){
        if(existProp("app_sg_buy_orderList","buy_id")) return true;
        $alt("请选择一个采购单进行明细处理！");
        return false;
     }
     return [
        {text:"增加原材料",iconCls:"gridItemAdd",
         handler:function(){
         	var sm = getCmp("app_sg_buy_orderList2").getSelectionModel();
         	sm.selectAll();
         	var d = getData("app_sg_buy_orderList");
         	var dd = getSelectedRows("app_sg_buy_orderList2");
         	var ddd = getData("app_sg_buy_orderList2",["buy_id","buyed_price","total_money"]);
         	var allPrice = 0.00;
         ...
         {text:"删除原材料",iconCls:"gridItemDel",
          handler:function(){
              var d=getData("app_sg_buy_orderList2",["detail_id","buy_id"]);
              if(!d.detail_id){
                  $alt("请选择一个采购单明细进行取消！");
                  return;
              }
                        
              doPost("/buy_order_list.delete.go",d,cLoadEvent("app_sg_buy_orderList2",d));
