...
public class tuicai extends HttpServlet {
	...
	List tuicailist=new ArrayList();
	request.setCharacterEncoding("GB2312");
	String SF_ID=request.getParameter("set_id");
	String ORDER_ID=request.getParameter("order_id");
	if(ORDER_ID==null&&SF_ID==null)
	{
		String aa=(String)session.getAttribute("SF_ID"); SF_ID=aa;System.out.println("ppppppppppppp="+SF_ID);
		String bb=(String)session.getAttribute("ORDER_ID"); ORDER_ID=bb;System.out.println("ppppppppppppp="+ORDER_ID);
	}
	else
	{
	...
	//更新已上退掉的的菜
	String checkbox[]=request.getParameterValues("tuicaicheckbox");
	String yuanyin=request.getParameter("yuanyin");
	System.out.println("原因="+yuanyin);
	 try{
		 ...
		 for(int i=0;i<checkbox.length;i++)
			{
				...
				System.out.println("错误");
			     ...
			     while(rs3.next())
			     {
					...
					String sql2="UPDATE order_list set FOOD_ACTION='2',FOOD_RETURN_TYPE='"+yuanyin+"' where  (ORDER_ID='"+ORDER_ID+"' and FOOD_ID='"+checkbox[i]+"' and FOOD_ACTION='1' and ORDER_LIST_ID='"+rs3.getString("ORDER_LIST_ID")+"')";
    ...
    try{
        ...
        while(rs.next()){
            ...
            tuicailist.add(order_list);
        }  
        request.setAttribute("tuicai",tuicailist);
        request.getRequestDispatcher("/lin/tuicai.jsp").forward(request, response);
        rs.close();   
        stmt.close();   
        conn.close();   
     }catch(Exception e){   
