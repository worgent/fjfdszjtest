...
<tr id="clientTr" style="display:none">
 ...
 <s:hidden value="%{order.ADDRESSID}" name="search.paddressid" id="paddressid"></s:hidden>
  ...
  <div id="paddressTip"></div>   
   ...
   <tr class="trex"> 
   <td class="tdex">预约时间:
   <s:textfield id="pbookingtime"  name="search.pbookingtime" value="%{order.BOOKINGTIME}" readonly="true"></s:textfield>(注:不选择为立即上门揽收)
...
<script language="javascript" type="text/javascript">	
		...
		$("#pname").formValidator({onfocus:"最多20字符",oncorrect:"√"}).inputValidator({min:1,max:20,onerror:"请重填姓名"});
		$("#ptel").formValidator({onfocus:"电话号码或小灵通,例059512345678",oncorrect:"√"}).regexValidator({regexp:"^\\d{10,12}$",onerror:"请重填电话号码"});
		$("#pmobile").formValidator({onfocus:"11位手机号",oncorrect:"√"}).regexValidator({regexp:"mobile",datatype:"enum",onerror:"请重填手机号"});
		$("#paddress").formValidator({onfocus:"取件地址",oncorrect:"√"}).inputValidator({min:1,onerror:"请重填取件地址"});
		$("#pmailtype").formValidator({onfocus:"邮件种类",oncorrect:"√"}).inputValidator({min:1,onerror:"请重填邮件种类"});
		...
		$("#porderingnum").formValidator({onfocus:"寄件数量",oncorrect:"√"}).regexValidator({regexp:"intege1",datatype:"enum",onerror:"请重填寄件数量"});
		$("#porderingweight").formValidator({onfocus:"物品重量",oncorrect:"√"}).regexValidator({regexp:"decmal4ex",datatype:"enum",onerror:"请重填物品重量"});
