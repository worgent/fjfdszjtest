	...
	<s:iterator id="g" value="%{pageList.objectList}">
		<tr>
			...
			<td>
			   ...
			   onclick="setCheck('<s:property value="#g.ID"/>','<s:property value="#g.PROVINCE"/>','<s:property value="#g.CITY"/>','<s:property value="#g.COUNTY"/>','<s:property value="#g.ADDRESS"/>','<s:property value="#g.ID" />','<s:property value="#g.PROVINCENAME" />', '<s:property value="#g.CITYNAME" />','<s:property value="#g.COUNTYNAME" />')"/>
			</td>
			<td>
...
//通过ajax方式,验证是否登录成功2010-04-14
	function setCheck(pid,pprovince,pcity,pcounty,paddress,id,provincename,cityname,countyname){
						...
						if (null != root){
							...
							if(0<value){//验证码错误
							   ...
							   //window.location.reload(true);
							    setValue(id,pprovince, provincename,pcity,cityname,pcounty,countyname,paddress);
