...
// 修改说明: 2009-12-07
// 1.增加两次封装,解决乱码问题:encodeURIComponent(encodeURIComponent(srcjo.val()));
// 2.search增加前缀
// 3.修改时间验证if(type=="datetime"){lb_go_on = isDateTime(val)};
// 4.下拉的时间控件在My97datePicker.htm被修饰引起了控件无法修改
// 5.修改交互的显示效果,提示信息除了出错,
//  其他都不提示setTipState方法的
// 2010-03-19:setTipState中启用,onshow,onerror属性,其他都不显示
// 显示颜色修改 onshow:黑色,onerror:红色加粗,修改样式文件validator.css
// 6.2010-04-03:增加属性onfocus,显示.结合上面修改,有onshow,onerror,onfocus属性,其他都不显示.
// 7.2010-04-07:还原早期设计,同时利用空串与空值组合达到显示效果.
//====================================================================================================
...
$.formValidator = 
{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		if(showmsg==null || showmsg==""||showmsg=="plonshow")
		{
			tip.hide();
		}
		else
		{
			if(initConfig.tidymode)
			{
				...
				elem.Tooltip = showmsg;
					if(showclass=="onError"||showclass=="onShow"||showclass=="onFocus"){
							tip.show();
					}
					else{
							tip.hide();
			...
			else if(showclass=="onError"||showclass=="onFocus"){
							tip.show();
			}else if(showclass=="onShow")
			{}
			else{
					tip.hide();
			}
	...
	*/
	...
	ajaxValid : function(returnObj)
	{
		...
		{
			//var parm = "clientid="+id+"&search."+id+"="+encodeURIComponent(encodeURIComponent(srcjo.val()));
			//ls_url = ls_url + (ls_url.indexOf("?")>0?("&"+ parm) : ("?"+parm));
			var parm = "clientid="+id+"&search."+id+"="+srcjo.val();
			ls_url = ls_url + ((ls_url).indexOf("?")>0?("&"+ parm) : ("?"+parm));
