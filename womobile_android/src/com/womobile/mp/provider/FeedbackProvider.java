...
public class FeedbackProvider  extends ContentProvider{
	...
	public static final String CONTENT_URI = "com.womobile.mp.provider.FeedbackProvider";
	...
	private static final String DATABASE_CREATE = "create table if not exists "
			+ DATABASE_TABLE_NAME + "("
			+ Feedback.KEY_ID + " varchar(20) , "
			+ Feedback.KEY_TITLE + " varchar(20) , "
			+ Feedback.KEY_DETAIL + " varchar(20) , "
			+ Feedback.KEY_ADJUNCT + " varchar(20) , "
			+ Feedback.KEY_ATTITUDE + " varchar(20) , "
			+ Feedback.KEY_DEFINE + " varchar(20) , "
			+ Feedback.KEY_FLOWER_STATE + " varchar(20) , "
			+ Feedback.KEY_PROCESS_CODE + " varchar(20) , "
			+ Feedback.KEY_AUDITING_CODE + " varchar(20) , "
			+ Feedback.KEY_BILL_STATE + " varchar(20) , "
			+ Feedback.KEY_CREATE_CODE + " varchar(20) , "
			+ Feedback.KEY_CREATE_DATE + " varchar(20) , "
			+ Feedback.KEY_EDITOR_CODE + " varchar(20) , "
			+ Feedback.KEY_EDITOR_DATE + " varchar(20) , "
			+ Feedback.KEY_BILLID + " varchar(20) "
			+")";
		
	private DatabaseHelper feedbackHelper = null;

	@Override
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		if (feedbackHelper == null)
			feedbackHelper = new DatabaseHelper(getContext());
		SQLiteDatabase db = feedbackHelper.getWritableDatabase();
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		SQLiteDatabase sqlDB = feedbackHelper.getWritableDatabase();
		long rowId = sqlDB.insert(DATABASE_TABLE_NAME, "", values);
		Log.i("rowId is", rowId + "");
		if (rowId > 0) {
			Uri rowUri = ContentUris.appendId(Feedback.CONTENT_URI.buildUpon(),
					rowId).build();
