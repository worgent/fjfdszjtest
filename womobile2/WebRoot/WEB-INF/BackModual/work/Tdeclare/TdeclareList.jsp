...
<html>
	...
	<body>
<s:form method='POST' name='form1' action="tdeclare"   namespace="/work"  id="form1" enctype="multipart/form-data">
		...
		<s:actionmessage theme="ems" />
						...
						<tr class="trex"  align="center">
							 <td  class="tdex" style="font-size:12px">主&nbsp;&nbsp;&nbsp;&nbsp;题：
								 <s:textfield id="ptitle" name="search.ptitle" size="30" value="%{tdeclare.title}" cssClass="editcss"></s:textfield> 
								 <div id="ptitleTip" style="display: inline"></div>
								 开始时间：
								 <s:textfield id="pbegmakedatetime" name="search.pbegmakedatetime" size="30" readonly="true" value="%{tdeclare.makedatetime}" cssClass="editcss"></s:textfield> 
								 <div id="pbegmakedatetimeTip" style="display: inline"></div>
								结束时间：
								 <s:textfield id="pendmakedatetime" name="search.pendmakedatetime" size="30" readonly="true" value="%{tdeclare.makedatetime}" cssClass="editcss"></s:textfield> 
								 <div id="pendmakedatetimeTip" style="display: inline"></div>
							</td>
						</tr>
						<tr class="trex">
							 <td  class="tdex" style="font-size:12px">登&nbsp;记&nbsp;人：
								 <s:textfield id="pmaker" name="search.pmaker" size="30" value="%{tdeclare.maker}" cssClass="editcss"></s:textfield> 
								 <div id="pmakerTip" style="display: inline"></div>
								 申报类别：
								<s:select id="pdeclaretype" name="search.pdeclaretype" value="%{tdeclare.declaretype}"   list="declaretypeList" listKey="id" listValue="name"  cssClass="selectcss"></s:select>
								 <div id="pdeclaretypeTip" style="display: inline"></div>
								 任务状态：
...
<script language="javascript" type="text/javascript">	
		    ...
		    //默认加载页面
		    loadSearchList('<%=path%>/work/tdeclare.do?action=listdetail','',1,1);
		    //验证
	...
	function add(){
		...
		window.location.href="<%=path%>/work/tdeclare.do?action=new";
	}
	...
	//数据修改
	function modify(pid){
	    window.location.href="<%=path%>/work/tdeclare.do?action=edit&search.pid="+pid;
	}
	...
	function del(pid){
	   jConfirm('您确定删除该信息!', '确认框', function(r) {	
	    		if (r){
				   var url = '<%=path%>/work/tdeclare.do?action=delete&search.pid='+pid
					try{
						...
						if (null != root){
							...
							if(0<rowSet.item(0).selectSingleNode("value").text){
							   window.location.href='<%=path%>/work/tdeclare.do';
							}
							else{
							    jAlert('删除失败!');  
							}
					...
					if(flag==1)
					{
					   ...
					   if(myvar==0){//删除 method方法0 代表删除
							jConfirm('您确定删除选中的!', '确认框', function(r) {
								    		if (r){
												var url = '<%=path%>/work/tdeclare.do?action=alldel&search.pid='+value
												try{
													var oXMLDom	= new ActiveXObject("Msxml.DOMDocument");
													oXMLDom.async = false ;
													oXMLDom.load(url);  
													...
													if (null != root){
														...
														if(0<rowSet.item(0).selectSingleNode("value").text){
														    //jAlert("删除成功!",fuction(r){
														     window.location.href='<%=basePath%>/work/tdeclare.do';
														    //});
					     ...
					     if(value.indexOf(",")==-1){
					        window.location.href="<%=path%>/work/tdeclare.do?action=edit&search.pid="+value;
					     }else{
					        jAlert( '修改时仅可选一项！','提示');
					     }
					   }else if(myvar==2){//审批
							jConfirm('您确定审核该单据吗?', '确认框', function(r) {
								    		if (r){
												            $.ajax({
												                url: "<%=path%>/work/tdeclare.do?search.pid="+value,
												                type: "POST",
												                data: { action:"auth"},
