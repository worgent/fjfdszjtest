...
public class FrontLoginFacadeImpl implements FrontLoginFacade {
	...
	public  HashMap checkLogin(HashMap map){
		...
		List ls=baseSqlMapDAO.queryForList("User.findSessionUserById", map);
		if(ls.size()==0){
			return null;
		}
		HashMap user=(HashMap) ls.get(0);
		if (user != null) {
			try {
				//对用户提交的密码进行md5加密处理
	...
	public  boolean regedit(HashMap map){
		...
		try {
			...
			//新增加默认联系人信息
			map.put("pis_check", "1");//是否默认
			map.put("pcreate_code",id);
			map.put("peditor_code",id);
			i=i+baseSqlMapDAO.update("ClientMsg.insertClientMsg", map);
