...
import com.qzgf.core.service.IBrandManager;
...
public class TypeAction extends WWAction {
	...
	private IBrandManager emsbrandManager;
	...
	public String checkname(){
		 ...
		 if(this.emsgoodsTypeManager.checkname( this.goodsType.getName(),goodsType.getType_id() )){
	...
	public String list() {
		this.webpage = this.emsgoodsTypeManager.pageType(order, this.getPage(),
				this.getPageSize());
		return "list";
	}
	...
	public String trash_list() {
		...
		this.webpage = this.emsgoodsTypeManager.pageTrashType(order, this
	...
	public String edit() {
		...
		this.goodsType = this.emsgoodsTypeManager.get(type_id);
		this.session.put(GOODSTYPE_SESSION_KEY, goodsType);
		this.is_edit = 1;
		return "edit";
	}
	...
	public String saveParams() {
		...
		String params = this.emsgoodsTypeManager.getParamString(paramnums,
	...
	public String saveProps() {
		...
		String props = this.emsgoodsTypeManager.getPropsString(propnames,
				proptypes, options);
	...
	public String save() {
		...
		this.emsgoodsTypeManager.save(tempType);
	...
	private String getResult() {
		...
		if (tempType.getHave_prop() != 0 && saveState.getDo_save_props() == 0) { // 用户选择了此类型有属性，同时还没有保存过
			...
			brandlist= this.emsbrandManager.list();
			result = "join_brand";
		} else {
	...
	public String delete() {
		try {
			int result  = emsgoodsTypeManager.delete(id);
			if(result ==1)
			this.json = "{'result':0,'message':'删除成功'}";
			else
			this.json ="{'result':1,'message':'此类型存在与之关联的商品，不能删除。'}";
	...
	public String disPropsInput() {
		attrList = this.emsgoodsTypeManager.getAttrListByTypeId(type_id);
	...
	public String disParamsInput() {
		this.paramAr = this.emsgoodsTypeManager.getParamArByTypeId(type_id);
		return "params_input";
	}
	...
	public String listBrand(){
		this.brandlist = this.emsgoodsTypeManager.listByTypeId(type_id);
		return "brand_list";
	}	
	...
	/**
	 * Purpose      : 说明
	 * @return the emsgoodsTypeManager
	 */
	public IGoodsTypeManager getEmsgoodsTypeManager() {
		return emsgoodsTypeManager;
	}
	...
	/**
	 * Purpose      : 说明
	 * @param emsbrandManager the emsbrandManager to set
	 */
	
	public void setEmsbrandManager(IBrandManager emsbrandManager) {
		this.emsbrandManager = emsbrandManager;
	}
