...
public class GoodsTypeManager extends BaseSupport<GoodsType> implements IGoodsTypeManager {
	...
	public List listAll() {
		String sql   = "select * from ems_goods_type where disabled=0";
	...
	public Page pageTrashType(String order,int page,int pageSize){
		...
		String sql  = "select * from ems_goods_type where disabled=1";
		sql+="  order by ";
		sql+=order;
	...
	public GoodsTypeDTO get(Integer type_id) {
		String sql ="select * from ems_goods_type where type_id=?";
	...
	private String getParamsByTypeId(int type_id) {
		...
		String sql ="select params from ems_goods_type where disabled=0 and type_id="+type_id;
	...
	public void save(GoodsType type) {
		String typeTableName ="ems_goods_type";
	...
	public int delete(Integer[] type_ids) {
		...
		String sql  ="select count(0) from " + this.getTableName("ems_goods") +" where type_id in ("+ids+")";
		int count = this.daoSupport.queryForInt(sql);
		if(count==0){
			sql  = "update  ems_goods_type set disabled=1  where type_id in ("+ids+")";
	...
	public void clean(Integer[] type_ids){
		...
		String sql  ="delete from ems_goods_type where type_id in("+ids+")";
		this.baseDaoSupport.execute(sql);
	...
	public void revert(Integer[] type_ids){
		...
		String sql  = "update  ems_goods_type set disabled=0  where type_id in ("+  ids +")";
		this.baseDaoSupport.execute(sql);
	}
	...
	public boolean checkname(String name, Integer typeid) {
		...
		String sql  ="select count(0) from ems_goods_type where name=? and type_id!=?";
