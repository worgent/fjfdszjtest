...
$(function(){
	$(".btn").click(function(){
		...
		top.location.href="http://"+$(this).attr("adomain") + ":8080/eop/admin";
		});
});
</script>
<div class="grid">
...
<grid:grid from="webpage">
	...
	<grid:body item="userSite">
		...
		<c:if test="${userSite.logofile!=null }">
				<img src="${staticserver }/${userSite.logofile }" width="100" height="56" />
			</c:if>
		</grid:cell>
		...
		<grid:cell><ul style="width:90%">
			<c:forEach var="item" items="${userSite.eopSiteDomainList }">
				<li style="width:100%;float:left">${item.domain } &nbsp;<c:if test="${defaultsiteid != userSite.id }"><a class="btn" href="javascript:;" adomain="${item.domain }">管理此站点</a></c:if></li>
			</c:forEach></ul>
		</grid:cell>
		<grid:cell>&nbsp;
