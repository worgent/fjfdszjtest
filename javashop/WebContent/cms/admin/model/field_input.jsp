	       ...
	       <td>
		       <select name="dataField.show_form" id="show_form">
		        <c:forEach items="${fieldPluginList}" var="plugin">
		        	<option haveSelectValue=${plugin.haveSelectValue } dataType="${plugin.dataType}"  value="${plugin.id }">${plugin.name }</option>
		        </c:forEach>  	
		       </select>	   
		       <input type="hidden" name="dataField.data_type" id="data_type" value="1"  />
		   </td>		   
	      </tr>	     
	      <!-- 
	     <tr>
...
$(function(){
	...
	$("#show_form option").click(function(){
		...
		$("#data_type").val($(this).attr("dataType"));
	});
	<c:if test="${isEdit==true}" >
	$("#data_type").val($(this).attr("${ dataField.data_type}"));
