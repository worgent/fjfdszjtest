...
function bindPhotoEvent(){
	...
	//绑定切换默认图片事件
	 ...
	 $(".proWrapper>a").click(function(){
		 var link = $(this);
		 viewPic(link.attr("img"),link.attr("wrapper"));
	 });
	...
	$.ajax({
		...
		success:function(result){
		   if(result.result==0){
				...
				if(defaultpic.attr("id").replace('img_','') == id.replace('container_','')) { //正在删除默认图片
