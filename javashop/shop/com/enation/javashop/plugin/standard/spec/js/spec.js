...
var Product= {
	...
	init:function(){
		...
		Eop.Dialog.init({id:"specdlg",title:"规格",width:"500px",height:'400px'});
	...
	createPros:function(specs,names){
		...
		for(i in products){
			...
			html+='<input type="text" name="stores" size="4" autocomplete="off"> </td>';
			
			html+='<td><input type="text" name="prices" size="8" autocomplete="off"></td>';
			html+='<td><input type="text" name="costs" size="8" autocomplete="off"></td>';
			html+='<td><input type="text" name="weights" size="10" autocomplete="off"></td>';
			html+='<td><a href="javascript:;" class="delete"></a></td>';
			html+="</tr>";
			var trEl =$(html);
			$("#productNode").append(trEl);
			trEl.find("a.delete").click(function(){
				self.deleteProRow($(this));
			});	 
		}
	},
